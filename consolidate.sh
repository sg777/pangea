...
if [ "${1}" = "" ]; then
	...
	enabled="y"
	maxInc="600" MinCheck="1" RawOut="[" OutAmount="0" tx_fee="0.001"
...
function makeRaw() {
    ...
    OutAmount=$(echo "scale=8; $OutAmount - $tx_fee" | bc) OutAmount=${OutAmount/#./0.} #0.001 is the transaction fee you wish to spend
...
if [[ $enabled == "y" ]]; then
    ...
    for ((tlc = 0; tlc <= $LoopsCount; tlc++)); do
        ...
        #echo "RawOut:: $RawOut"
        SECONDS=0
	echo "OutAmount :: $OutAmount"
        if [[ $OutAmount > $tx_fee ]]; then 
		toSign=$(./chips-cli  createrawtransaction $RawOut)
		echo "$toSign"
	        #addnlocktime
        	Signed=$(echo -e $toSign | ./chips-cli -stdin signrawtransactionwithwallet $newhex | jq -r '.hex')
